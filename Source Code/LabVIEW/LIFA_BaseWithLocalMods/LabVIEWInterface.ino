...
long retVal;            // changed to long Jan/2015
unsigned long triggerInterval=0, watchedStepper=0, moving=0; // jmr 4/2014  //changed to unsigned long Jan/2015
unsigned long triggerOff=0; // jmr 8/2014
unsigned char triggerPin; // jmr 4/2014
...
// addded jmr 8/2014
// if motor is doing a move (indicated by nonzero distancetogo) and is sitting on a camera trigger
// position, set the trigger pin high for 5 ms. Do this before the call to run()
// so we get a trigger at the initial position before starting to move. 

  if (triggerInterval) {  
    if (steppers[watchedStepper].distanceToGo() && !(_I32_abs(steppers[watchedStepper].currentPosition())%triggerInterval) ) {
      triggerOff=micros()+5000; moving=1;
    }
    if (micros()<triggerOff) {
...
// jmr 8/2014  
// when we get to the end of the movement, fire another trigger if we are
// on a trigger position
  if (triggerInterval) {
    if (!steppers[watchedStepper].distanceToGo() && moving) {
      moving=0;
      if ( (steppers[watchedStepper].currentPosition())%triggerInterval == 0 )triggerOff=micros()+5000;
    }
  }
  ...
  if(command[0] == 0xFF && checksum_Test(command) == 0)
  {
    ...
    *********************************************************************************/        
      ...
      case 0x33:  // Stepper steps to go
        ...
        Serial.write( ((retVal >> 16) & 0xFF) );  // Use 32 bit data for new motor
        Serial.write( ((retVal >> 24) & 0xFF) );
        break; 
        
        // added by jmr 4/2014
        
      case 0x40:  // Stepper current position
        retVal = steppers[command[2]].currentPosition();
        Serial.write( (retVal & 0xFF) );
        Serial.write( ((retVal >> 8) & 0xFF) );
        Serial.write( ((retVal >> 16) & 0xFF) );  // Use 32 bit data for new motor
        Serial.write( ((retVal >> 24) & 0xFF) );
        ...
        break; 
      case 0x41:  // Stop moving
        steppers[command[2]].move(0);
        Serial.write('0');
  
        break; 
      case 0x42:  // configure camera trigger interval
        watchedStepper = command[2];
        triggerPin = command[3];
        triggerInterval = (command[4]<<8) + command[5];
        Serial.write('0');
  
        break; 
      case 0x43:  // reset stepper position
        steppers[command[2]].setCurrentPosition( (((long)command[3])<<24) + (((long)command[4])<<16) + (((long)command[5])<<8) + command[6] );  // use 32 bit data for new motor
        Serial.write('0');
        
        break; 

      case 0x44:  // just send a camera trigger
        triggerOff=micros()+5000;
        if (micros()<triggerOff) {
          digitalWrite(triggerPin,HIGH);
        } else {
          digitalWrite(triggerPin,LOW);
...
// Stepper Functions
  ...
  void AccelStepper_Write(unsigned char command[]){
    long steps = 0;       // Changed to long Jan/2015
    int step_speed = 0;
    int acceleration = 0;
  
    
    // Changed to 32 bit data for steps of new 1.8 degree motor //Number of steps & speed are a 16 bit values, split for data transfer. Reassemble 2 bytes to an int 16
    steps = (((long)command[5]) << 24) + (((long)command[6]) << 16) + (((long)command[7]) << 8) + command[8]; 
    step_speed = (int)(command[2] << 8) + command[3];
    acceleration = (int)(command[9] << 8) + command[10];  //Changed to [9] [10] because of the increased data length of "steps"
