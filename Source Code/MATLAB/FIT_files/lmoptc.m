...
%
%Usage:
%   [p,iter,res,er,J,succ]=lmoptc(sys,Bs,obs,nbr,p0,W,np,cpar)
%
%where       
...
miter=25; lim=500;   % miter=25  lim=100
n=length(p0);
p=p0(:);
if nargin==5
  W=1; np=8; cpar=[];
...
for i=1:lim
  ...
  [f,J]=jacobc(sys,Bs,nbr,p,np,cpar);
  er=y-f; res=er'*W*er;
  if abs(res-pres)<1e-6 && i>miter, succ=1; break; end  %1e-4, 1e-6
  if res<pres
    lambda=0.1*lambda;
    if lambda<1e-7,lambda=1e-7; end;  %1e-7
    beta=J'*W*er; alpha=J'*W*J;
    pres=res; pp=p;
  else
    lambda=10*lambda;
